...
def test_node_init(node):
    assert_equal(node.ref_location, [4, 5, 6])
...
def test_node_ref_x(node):
    node.ref_x = 9

    assert_equal(node.ref_x, 9)
    assert_equal(node.ref_location, [9, 5, 6])
    assert_equal(node.location, [4, 5, 6])


def test_node_ref_y(node):
    node.ref_y = 9

    assert_equal(node.ref_y, 9)
    assert_equal(node.ref_location, [4, 9, 6])
    assert_equal(node.location, [4, 5, 6])


def test_node_ref_z(node):
    node.ref_z = 9

    assert_equal(node.ref_z, 9)
    assert_equal(node.ref_location, [4, 5, 9])
    assert_equal(node.location, [4, 5, 6])
...
def test_node_ref_location(node):
    node.ref_location = [3, 2, 1]

    assert_equal(node.ref_location, [3, 2, 1])
    assert_equal(node.location, [4, 5, 6])
...
def test_node_support_xyz(node):
